...
@router.post('', summary="权限创建", response_model=base.BaseResp, dependencies=[Security(check_permissions)])
async def create_access(req: Request, post: role.CreateAccess):
    """
    创建权限
    ...
    if not req.state.user_type:
        return fail(msg="只有超级管理员才可以创建权限!")
    ...
    return success(msg=f"权限 {result.access_name} 创建成功!")
